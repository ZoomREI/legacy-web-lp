...
class DMS_Seo_Yoast extends Abstract_Seo {
	...
	public function admin_styles() {
		wp_register_style( 'dms-yoast-min-css', $this->dms_instance->plugin_url . 'assets/css/dms-yoast.min.css', array(), $this->dms_instance->version, 'all' );
	...
	public function admin_scripts() {
		...
		wp_register_script( 'dms-yoast-js', $DMS->plugin_url . 'assets/js/dms-yoast.js', array(
			'jquery'
		), $DMS->version );
	...
	public function overrideHead() {
		...
		if ( ! empty( $dms_instance->active_mapping ) && ! empty( $dms_instance->real_requested_object_id )
		     && $dms_instance->dms_fs->can_use_premium_code__premium_only() ) {
	...
	public function setupSitemapFilters() {
		...
		// Rewrite each object entry in the sitemap
		add_filter( 'wpseo_sitemap_entry', function ( $url, $type, $object ) {
			return $this->rewriteEntry( $url, $type, $object, true );
		}, 10, 3 );
	 ...
	 * @return array
	 */
	public function rewriteEntry( $url, $type, $object, $removeExisting = false ) {
		$DMS = $this->dms_instance;
		if ( $object instanceof WP_Post ) {
			...
			if ( $object->taxonomy == 'category' ) {
				...
				$key = implode( '#', [
					$object->taxonomy,
					$object->slug,
					get_taxonomy( $object->taxonomy )->object_type[0]
				] );
			}
		}
	...
	public function getPostTabContent( $arg1 ) {
		...
		if ( ! empty( $post->ID ) ) {
			...
			foreach ( $mappings as $key => $item ) {
				...
				// Get meta data
				$data[] = [ 'host_path' => $host_path, 'meta_data' => $this->getDataPost( $host_path, $post->ID ) ];
			}
			ob_start();
	...
	public function getTaxonomyTabContent( $arg1 ) {
		...
		if ( ! empty( $current_screen ) && is_a( $current_screen, 'WP_Screen' ) && ! empty( $current_screen->taxonomy ) && ! empty( $tag_ID ) ) {
			...
			foreach ( $mappings as $key => $item ) {
				...
				$host_path = DMS_Helper::getHostPlusPath( $item );
				// Get meta data
				$data[] = [ 'host_path' => $host_path, 'meta_data' => $this->getDataTaxonomy( $host_path, $tag_ID ) ];
			}
			ob_start();
			require_once $DMS->plugin_dir . '/templates/seo/yoast/settings.php';
			$content = ob_get_contents();
	...
	public function saveMetaForPost( $post_id ) {
		if ( $this->dms_instance->dms_fs->can_use_premium_code__premium_only() ) {
			$data = DMS_Helper::getMappingsByValue( $this->dms_instance->wpdb, $post_id, ARRAY_N );
			foreach ( $data as $value ) {
				// Get host + path as an url part. Only if mapping is numeric array
				$host_and_path = DMS_Helper::getHostPlusPath( $value );
	...
	public function saveMetaForTaxonomy( $term_id, $term_taxonomy = null ) {
		if ( $this->dms_instance->dms_fs->can_use_premium_code__premium_only() ) {
			...
			if ( $term->taxonomy === 'category' ) {
				...
				$value    = implode( '#', [ $term->taxonomy, $term->slug, $taxonomy->object_type[0] ] );
			}
			$data = DMS_Helper::getMappingsByValue( $this->dms_instance->wpdb, $value, ARRAY_N );
			foreach ( $data as $value ) {
				// Get host + path as an url part. Only if mapping is numeric array
				$host_and_path = DMS_Helper::getHostPlusPath( $value );
				// Replacing because post key value couldn't be white '.'.
	...
	public function isSitemapRequested() {
		...
		return $this->isInteractingWithSitemap()
		       || DMS_Helper::endsWith( $DMS->getPath(), '/' . self::SITEMAP_FILENAME )
		       || preg_match( self::SITEMAP_OBJECT_PATTERN, $DMS->getPath() );
	}
}