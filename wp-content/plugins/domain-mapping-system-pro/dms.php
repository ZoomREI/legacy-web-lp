...
/*
 * Plugin Name: Domain Mapping System (PRO)
...
if ( !function_exists( 'dms_fs' ) ) {
    ...
    if ( dms_fs() ) {
        ...
        add_filter(
            'rest_url',
            array('DMS', 'changeWordpressApiUrl'),
            99,
            3
        );
        add_filter( 'wp_page_menu_args', array('DMS', 'removeHomeLinkFromNavigation'), 99 );
        add_filter(
            'wp_list_pages',
            array('DMS', 'addHomeUrlToNavigation'),
            99,
            3
        );
        add_action( 'admin_init', array('DMS', 'updateSunriseFile') );
