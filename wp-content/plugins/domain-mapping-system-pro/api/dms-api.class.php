...
class DMS_API {
	...
	public function insertMapping( $data ) {
		if ( $data->get_header( 'Content-Type' ) !== 'application/json' ) {
			return self::errorResponse( 'invalid_content_type',
				sprintf( __( 'Invalid content type. Please check our documentation %s', 'dms' ),
		...
		if ( $parsed_host != $item['host'] || strpos( $item['host'],
				'.' ) === false || 'http://' . $item['host'] != esc_url_raw( $item['host'] ) ) {
		...
		if ( $instance->dms_fs->can_use_premium_code__premium_only() ) {
			if ( isset( $item['path'] ) ) {
				...
				$parsed_path  = wp_parse_url( 'http://' . $item['host'] . '/' . $item['path'],
					PHP_URL_PATH );
				if ( $parsed_path != '/' . $item['path'] || 'http://' . $item['host'] . '/' . $item['path'] != esc_url_raw( $item['host'] . '/' . $item['path'] ) ) {
					return self::errorResponse( 'invalid_paths_specified', __( 'Invalid paths specified', 'dms' ) );
				}
			}
		} else {
			$item['path'] = null;
		...
		if ( $item['host'] === DMS_Helper::getBaseHost() ) {
			return self::errorResponse( 'unable_to_map_primary_domain',
				sprintf( __( 'Unable to map primary domain. Please check our documentation %s', 'dms' ),
					$this->documentation_link ) );
		}
		...
		$id = $wpdb->get_row( $wpdb->prepare( 'SELECT `id` FROM ' . $wpdb->prefix . 'dms_mappings WHERE `host`="%s" AND `path`' . ( ! empty( $item['path'] ) ? '="%s"' : ' IS NULL' ),
			$args ) );

		// If exists then update
		if ( ! empty( $id ) ) {
			$mapping = $wpdb->get_row( $wpdb->prepare( 'SELECT * FROM ' . $wpdb->prefix . 'dms_mapping_values WHERE `host_id`=%d AND `value`= %s',
				[
					$id->id,
			...
			if ( ! empty( $mapping ) && (int) $mapping->primary == rest_sanitize_boolean( $item['primary'] ) ) {
				return self::errorResponse( 'duplicate_entries_detected_with_the_same_root_domain_and_subdirectory',
					__( 'Duplicate entries detected with the same root domain and subdirectory', 'dms' ) );
			} elseif ( ! empty( $mapping ) && rest_sanitize_boolean( $item['primary'] ) ) {
				$wpdb->update( $wpdb->prefix . 'dms_mapping_values', [ 'primary' => 0 ], [ 'host_id' => $id->id ] );
				$wpdb->update( $wpdb->prefix . 'dms_mapping_values', [ 'primary' => 1 ], [
					'host_id' => $id->id,
					'value'   => sanitize_text_field( $item['value'] )
				] );

				return self::response( [ 'message' => __( 'Successfully saved', 'dms' ) ] );
	...
	private static function insertMappingValues( $item, $host_id, $instance ) {
		...
		$insert_result = $instance->prepareMappingInsert( sanitize_text_field( $item['value'] ), $item, $host_id,
			$ok_values );
		...
		if ( $insert_result = 1 || $insert_result = 2 ) {
			...
			if ( $instance->dms_fs->can_use_premium_code__premium_only() && ! empty( $dms_archive_global_mapping ) ) {
				...
				if ( DMS::isTaxonomyTerm( sanitize_text_field( $item['value'] ) ) ) {
					...
					$posts         = DMS::getPostsByTaxonomyTermId( $term_taxonomy[0],
						$term->term_id );
					foreach ( $posts as $post_id ) {
						$insert_result = $instance->prepareMappingInsert( $post_id, $item,
							$host_id, $ok_values );
						if ( $insert_result === 0 ) {
							break;
						}
				...
				if ( $shop_page_association == sanitize_text_field( $item['value'] ) ) {
					...
					if ( ! empty( $products ) ) {
						foreach ( $products as $product_id ) {
							$insert_result = $instance->prepareMappingInsert( $product_id, $item,
								$host_id, $ok_values );
							if ( $insert_result === 0 ) {
								break;
							}
	...
	public function token( $data ) {
		try {
			...
			$secret = is_multisite() ? get_blog_option( get_current_blog_id(),
				'dms_api_secret' ) : get_option( 'dms_api_secret' );
			if ( empty( $secret ) ) {
				return self::errorResponse( 'unable_to_get_token',
					sprintf( __( 'Unable to get token. Please contact the site administrator owner and review our documentation %s' ),
						$this->documentation_link ) );
			}
			$header  = json_encode( [ 'typ' => 'JWT', 'alg' => 'HS256' ] );
			$payload = json_encode( [ 'user_id' => $signon->data->ID ] );
			...
			// Technical error
			return self::errorResponse( 'technical_error',
				sprintf( __( 'Technical error. Please contact support and review our documentation %s', 'dms' ),
					$this->documentation_link ), 500 );
		}
	}
	 ...
	 * @return WP_Error
	 */
	public static function errorResponse( $code, $message, $httpStatusCode = 400, $data = null ) {
		$dataToSent = array( 'status' => $httpStatusCode );
		if ( is_array( $data ) ) {
