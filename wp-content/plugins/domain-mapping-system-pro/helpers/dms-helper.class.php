...
# END DMS plugin';
	...
	/**
	 * Get child page based on parent id and child's possible slug
	 *
	 * @param $parent_page_id
	 * @param $possible_child_slug
	 *
	 * @return array|false|WP_Post|null
	 */
	public static function getChildByParentAndSlug($parent_page_id, $possible_child_slug) {
		if( empty( $possible_child_slug ) ) {
			return false;
		}
		$child_page_url = get_page_link( $parent_page_id ) . trim( $possible_child_slug, '/' );

		return get_page_by_path( wp_parse_url( $child_page_url, PHP_URL_PATH ) );
	}

	/**
	 * Check if current page has mapped parent page or not
	 *
	 * @param $wpdb
	 *
	 * @return bool|array|string
	 */
	public static function hasMappedParent( $wpdb ) {
		global $post;
		if ( ! $post->post_parent ) {
			return false;
		}
		$mapping = self::getMappingsByValue( $wpdb, $post->post_parent, ARRAY_A, [ '`path`', '`host`', '`id`' ] );

		return ! empty( $mapping ) ? $mapping : false;
	}

	/**
	 * Get primary parent path of the mapping
	 *
	 * @param $wpdb
	 * @param $mapping
	 *
	 * @return false|string
	 */
	public static function getPrimaryParentMappingPath( $wpdb, $mapping_id ) {
		// Get the matching primary mapping value
		$primary_parent = $wpdb->get_row( $wpdb->prepare( 'SELECT `value` FROM ' . $wpdb->prefix . 'dms_mapping_values WHERE `host_id` = %s AND  `primary` = 1', $mapping_id ) );
		$value          = ! empty( $primary_parent ) ? $primary_parent->value : '';
		if ( ! empty( $value ) ) {
			$permalink = ( $value == get_option( 'page_on_front' ) ) ? site_url( '/' . get_post( $value )->post_name ) : get_permalink( $value );

			return trim( wp_parse_url( $permalink, PHP_URL_PATH ), '/' );
		}

		return false;
	}
	/**
	 * Check is the current match with any secondary mapping
	 *
	 * @param $path
	 * @param $mappings
	 *
	 ...
	 */
	public static function isSecondaryTerm( $path, $mappings ) {
		if ( ! empty( $path ) && count( $mappings ) > 1 ) {
			$secondary_mappings = array_filter( $mappings, function ( $item ) {
				return str_contains( $item['value'], 'term' );
			} );
			foreach ( $secondary_mappings as $mapping ) {
				$term_id   = (int) preg_replace( '/^term_/', '', $mapping['value'] );
				$term_link = get_term_link( $term_id );
				if ( ! is_wp_error( $term_link ) ) {
					$term_path = trim( wp_parse_url( $term_link, PHP_URL_PATH ), '/' );
					if ( $term_path == $path ) {
	...
	/**
	 * Check does the post belong to any mapped term
	 *
	 * @param $key
	 *
	 * @return false|mixed
	 */
	public static function isMappedCPT( $key ) {
		global $wpdb;
			...
			foreach ( $taxonomies as $taxonomy ) {
				...
				if ( ! empty( $terms ) ) {
					foreach ( $terms as $term ) {
						$mappings = self::getMappingsByValue( $wpdb, 'term_' . $term->term_id );
						if ( ! empty( $mappings ) ) {
	...
	/**
	 * Get custom taxonomies of the given post type
	 *
	 * @param $object_type
	 *
	 * @return array|string[]
	 */
	public static function getCustomTaxonomies( $object_type ) {
	...
	/**
	 * Get post/term/category title by mapping value
	 *
	 * @param $value
	 *
	 * @return string
	 */
	public static function getTitleOfValue( $value ) {
		if ( str_starts_with( $value, 'category' ) ) {
			return get_category_by_slug( str_replace( 'category-', '', $value ) )->name;
		} else if ( str_starts_with( $value, 'term' ) ) {
			return get_term( str_replace( 'term_', '', $value ) )->name;
		} else {
	 ...
	 * @return mixed
	 */
	public static function checkMappingExistance( $wpdb, $host_id, $value, $primary, $path ) {
		$value_placeholder = is_numeric( $value ) ? '%d' : '%s';
		$path_placeholder  = empty( $path ) ? 'IS NULL' : '%s';

		$query = $wpdb->prepare(
			"SELECT * FROM {$wpdb->prefix}dms_mapping_values 
         WHERE `host_id` = %d AND `value` = {$value_placeholder} 
         AND `primary` = %d AND `value_permalink_path` {$path_placeholder}",
			$host_id,
			$value,
			$primary
		);
