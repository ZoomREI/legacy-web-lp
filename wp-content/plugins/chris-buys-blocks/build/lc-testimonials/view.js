  ...
  const initializeCarousel = () => {
    if (window.innerWidth < 1024) {
      ...
      const moveToSlide = targetIndex => {
        ...
        const targetPosition = targetSlide.offsetLeft - (track.clientWidth - slideWidth) / 2;
        isScrolling = true; // Set the flag to true before starting the scroll
        gsap__WEBPACK_IMPORTED_MODULE_0__["default"].to(track, {
          scrollTo: {
            x: targetPosition
          },
          duration: 0.6,
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (window.innerWidth >= 1024) {
    const loadMoreButton = document.querySelector(".lc-testimonials__load-more");
