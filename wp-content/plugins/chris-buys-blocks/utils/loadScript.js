function loadScript(src, callback = () => {}) {
	...
	let shouldLoad = src.filter(
		(item) =>
			!window.loadedScripts.includes(item.replace(/&callback=[^&]*/, "")),
	);
	let hasCallbackInURL = src.find((item) => item.indexOf("&callback=") > -1);
	let allScriptsLoaded = src.every((script) =>
		window.fullyLoadedScripts.includes(script.replace(/&callback=[^&]*/, "")),
	);
	...
	if (allScriptsLoaded) {
		...
		let notFullyLoaded = src
			.map((item) => item.replace(/&callback=[^&]*/, ""))
			.filter((item) => !window.fullyLoadedScripts.includes(item));
	...
	window.loadedScripts = Array.from(
		new Set([
			...window.loadedScripts,
			...shouldLoad.map((item) => item.replace(/&callback=[^&]*/, "")),
		]),
	);
		...
		script.onload = function () {
			...
			if (window.loadingQueue.length) {
				window.loadingQueue.forEach((item, index) => {
					...
					let thisAllScriptsLoaded = item.scripts.every((script) =>
						window.fullyLoadedScripts.includes(
